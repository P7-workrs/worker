...
using p7Worker.ContainerHandling.Interfaces;
        ...
        while(true)
        {
            if (_handler.IsConnected)
            {
                ...
                if (DateTime.UtcNow - _keepAliveDt > _keepAliveTs)
                {
                    ...
                    WorkerReportDTO workerReport = new WorkerReportDTO(WorkerInfoDto.WorkerId, _ServiceTask != null ? _ServiceTask.Id : Guid.Empty);
